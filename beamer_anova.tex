...
\begin{itemize}
    ...
    \visible<2->{\item $H_0$: $\mu_A^* \geq \mu_B^*$ (Test if the treatment is better),}
...
\vspace{-.35cm}
	\begin{figure}[!tbp]
    \includegraphics[width=0.87\textwidth, clip, trim={0cm 0.2cm 0cm .5cm}]{rejet.pdf}
    {\tiny{$\mu_A^*=3,\ \mu_B^*=7$, we reject the equality.}}
\end{figure}
\vspace{-.45cm}
\begin{figure}
    \includegraphics[width=0.87\textwidth, clip, trim={0cm 0.2cm 0cm .5cm}]{not_rejet.pdf}
    {\tiny{$\mu_A^*=2, \ \mu_B^*=2.5$, we don't reject the equality.}}
\end{figure}
...
$X=[\ind_{c_1},\dots,\ind_{c_I}]\in \mathbb{R}^{n\times I}$:

\[\frac{1}{n-rg(X)} \norm{y-X\hat{\beta}^{LS}}^2 \text{ unbiased estimator of } \sigma^2\]

$\sum\limits_{i=1}^{I}\ind_{c_i}=\ind_n$
$rg(\Tilde{X})=I,\Tilde{X}=[\ind_n,\ind_{c_n},\dots,\ind_{c_I}]$
\end{alertblock}
\end{frame}
